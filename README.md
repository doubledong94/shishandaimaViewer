...
面板左侧有三种选择节点的方式，（1）按照节点在正则表达式中的位置选择节点，（2）按照节点所属的**普通字符**选择节点，（3）按照节点发生在哪个函数来选择节点     
选择左侧的筛选规则后，右侧会出现对应的节点列表，选择右侧的节点后，使用快捷键 `esc` 退出面板，就可以选中节点了。   
3. 快捷键 `ctrl + k` 按照节点类型选择，比如选择所有类型为 fieldOf 的节点
4. 快捷键 `ctrl + a` 选择所有节点
5. 快捷键 `c` 取消选择所有节点
6. 快捷键 `shift + /` 反向选择
7. 快捷键 `l + b` 查找连线并选中连线，如果不存在连线，则不新增选中的节点
8. 快捷键 `l + u` 向上查找共同父节点，并选中连线，如果不存在共同父节点，则不新增选中的节点
9. 快捷键 `l + d` 向下查找共同子节点，并选中连线，如果不存在共同子节点，则不新增选中的节点
10. 快捷键 `ctrl + ;` 按照度选中节点
11. 快捷键 `ctrl + ,` 按照度入选中节点
12. 快捷键 `ctrl + .` 按照度出选中节点
13. 快捷键 `.` 沿着已经选中的节点向上选择节点
14. 快捷键 `,` 沿着已经选中的节点向下选择节点
15. 快捷键 `p` 按照联通子图选中节点
16. 快捷键 `ctrl + p` 按照聚合选中节点
### 图布局
1. 快捷键 `z` 开关布局动画，由于布局计算非常耗时，因此设置这个开关。
2. 快捷键 `2` 2d布局
3. 快捷键 `3` 3d布局
4. 快捷键 `t + +` 增加布局温度，布局温度越高，布局速度越快，但也会导致节点抖动幅度越大
5. 快捷键 `t + -` 减少布局温度
6. 快捷键 `w + +` 增加选中的节点所组成的子图的布局权重，权重越大，布局月紧凑
7. 快捷键 `w + -` 减少选中的节点所组成的子图的布局权重
8. 快捷键 `ctrl + alt + w` 恢复未选中的节点布局权重
9. 快捷键 `ctrl + v` 固定选中的节点的位置
10. 快捷键 `shift + v` 取消固定选中的节点的位置
11. 快捷键 `ctrl + alt + v` 取消固定未选中的节点的位置
12. 快捷键 `ctrl + x` 固定选中的节点的x坐标
13. 快捷键 `shift + x` 取消固定选中的节点的x坐标
14. 快捷键 `ctrl + alt + x` 取消固定未选中的节点的x坐标
15. 快捷键 `ctrl + y` 固定选中的节点的y坐标
16. 快捷键 `shift + y` 取消固定选中的节点的y坐标
17. 快捷键 `ctrl + alt + y` 取消固定未选中的节点的y坐标
18. 快捷键 `ctrl + b` 限制选中的节点的位置范围
19. 快捷键 `shift + b` 取消限制选中的节点的位置范围
20. 快捷键 `ctrl + alt + b` 取消限制未选中的节点的位置范围
21. 快捷键 `ctrl + j` 聚合选中的节点
22. 快捷键 `shift + j` 取消聚合选中的节点
23. 快捷键 `ctrl + alt + j` 取消聚合未选中的节点
24. 快捷键 `x + l` 自动将出度和入度都是一的节点按照X坐标聚合
25. 快捷键 `y + l` 自动将出度和入度都是一的节点按照Y坐标聚合
26. 快捷键 `j + l` 自动聚合出度和入度都是一的节点
27. 快捷键 `b + c` 自动按照类来限制所有节点的位置范围
28. 快捷键 `b + m` 自动按照函数来限制所有节点的位置范围
### 图样式(大小与颜色)
...
9. 快捷键 `i + +` 连线变粗
10. 快捷键 `i + -` 连线变细
11. 快捷键 `N + +` 未选中的节点变的更不透明
12. 快捷键 `N + -` 未选中的节点变的更透明
13. 快捷键 `M + +` 选中的节点变的更不透明 
14. 快捷键 `M + -` 选中的节点变的更透明 
15. 快捷键 `h` 给选中的节点选择颜色，会覆盖节点之前的颜色
16. 快捷键 `ctrl + h` 给所有节点自动上色，不会覆盖节点之前的颜色
17. 快捷键 `ctrl + alt + h` 清除未选中的节点的颜色
### 删除节点和边
快捷键 `ctrl + /` 删除选中的节点     
快捷键 `ctrl + alt + /` 删除未选中的节点。bug：这个操作会留下一个节点，请重复使用两次以删除所有节点。    
删除的节点可以使用快捷键 `ctrl + f` 恢复     
快捷键 `ctrl + r` Transitive Recutct 
### 探索模式
使用正则搜索一次后，通过对搜索结果的阅读，会让你对代码有新的认识，因此会产生新的搜索需求。如果这新的搜索需求是针对图中的某些节点，那么不需要回到正则编辑面板，直接选中这些节点后，使用下面的快捷键进行二次搜索。    
快捷键 `shift + .` 选择正则，并以选中的节点为始端进行二次搜索   
快捷键 `shift + ,` 选择正则，并以选中的节点为末端进行二次搜索    
